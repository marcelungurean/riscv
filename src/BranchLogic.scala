...
class BranchLogic ( val expr_len : Int ) extends Module {
	...
	val io = new Bundle { 
		...
		val pc_sel 		= UInt ( OUTPUT , PC_SEL_WIDTH )
	}
...
class BranchLogicTest ( c : BranchLogic, val steps : Int ) extends Tester (c) {
	...
	// println (s"BranchLogicTest: $steps_passed / $steps passed !" )
	println (s"BranchLogicTest: passed !" )
	println (s"BranchLogicTest: TO DO : Check the BranchLogic !" )
